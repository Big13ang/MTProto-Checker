...
func TestRecoverMiddlewareTurnsPanicInto500(t *testing.T) {
	...
	if body.Error.Message != "internal error" || body.Error.Code != "INTERNAL_SERVER_ERROR" || body.Success {
...
func TestEndpointsRejectNonPost(t *testing.T) {
	for _, path := range []string{"/check", "/check-batch", "/check-stream"} {
		rec := httptest.NewRecorder()
		NewMux("test").ServeHTTP(rec, httptest.NewRequest(http.MethodGet, path, nil))
...
func TestEndpointsRejectCrossOriginPosts(t *testing.T) {
	for _, path := range postPaths {
		for _, site := range []string{"cross-site", "same-site"} {
			...
			} else if body.Error.Code != "FORBIDDEN" || body.Success {
				t.Errorf("POST %s: body = %q, want FORBIDDEN", path, rec.Body.String())
			}
		}
	}
}
...
func TestCheckRejectsMalformedJSON(t *testing.T) {
	...
	if body.Success || body.Error.Code != "BAD_REQUEST" {
		t.Errorf("expected success=false and code=BAD_REQUEST, got: %+v", body)
	}
}
...
func TestEndpointsRejectOversizedBody(t *testing.T) {
	...
	for _, c := range cases {
		...
		if !strings.Contains(body.Error.Message, "exceeds") || body.Error.Code != "PAYLOAD_TOO_LARGE" {
			t.Errorf("POST %s error = %+v, want it to mention the limit", c.path, body)
		}
	}
}
...
func TestRootEndpoint(t *testing.T) {
	rec := httptest.NewRecorder()
	NewMux("test").ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/", nil))
